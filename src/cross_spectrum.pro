FUNCTION CROSS_SPECTRUM, x, y, DX=dx, DY=dy, DT=dt, N_SEG=n_seg, $ 
                         ...
                         NN=nn, ALL_COV=covar_all, ALL_ERR_COV=cov_all_err, LEAHY=leahy
...
;                           output
;       18/01/14  - v3.9 - added warning for -ve covariance (due to -ve coherence
;                           or PSD after noise subtraction).
;       21/01/14  - v3.91 - additional check for covar_all < 0.
;       21/07/14  - v4.0  - added LEAHY input keyword
;
; USES:
;       REGROUP
;
...
; Leahy et al. (1983) style normalisation, when /LEAHY is used
  ...
  if KEYWORD_SET(leahy) then begin
    ...
    means_1 = TRANSPOSE(REBIN(mean_1, m, nf, /sample))
    means_2 = TRANSPOSE(REBIN(mean_2, m, nf, /sample))
    norm1 = norm1 / SQRT(means_1)
    norm2 = norm2 / SQRT(means_2)
  endif
...
; Error formula (Phil Uttley - priv. comm. July 2013)
  ...
  cov_err = SQRT((nn * (f_u - f_l) / binS_2 / 2.0) > 0)

; ----------------------------------------------------------
; compute the covariance over all frequencies (a useful check)
; XXX still under development XXX
  ...
  covar_all = SQRT(covar_all > 0)
  
  cov_all_err = (S_1_all * N_2_all * delta_f) + $
                (S_2_all * N_1_all * delta_f) + $
                (N_1_all * N_2_all * delta_f)

  cov_all_err2 = nn_all * delta_f / S_2_all / 2.0
  cov_all_err = SQRT(cov_all_err2 > 0)
